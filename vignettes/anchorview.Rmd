...
Now we need to associate anchors with clusters in the two datasets.

```{r}
extract_cluster_anchors(clustered_anchors, clusters1="seurat_clusters", clusters2="seurat_clusters") -> extracted_cluster_anchors

head(extracted_cluster_anchors)
```

Now we can plot this in a couple of ways.

```{r}
plot_cluster_interaction_matrix(extracted_cluster_anchors)
```
