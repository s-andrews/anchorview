...
original_data %>%
  ...
  ScaleData() %>%
  RunPCA() %>%
  RunUMAP(dims = 1:30, reduction = "pca", reduction.name = "umap.unintegrated") -> original_data
```
...
```{r}
get_positions_for_anchors(default_anchors, reduction = "umap.unintegrated") -> default_anchor_positions
...
This is good but is overcrowded.  We can subsample to show fewer anchors

```{r fig.height=7, fig.width=8}
plot_integration_anchors(
  split_objects,
  default_anchor_positions %>% sample_n(500),
  reduction="umap.unintegrated"
) + theme_bw()
```
...
```{r}
cluster_anchor_positions(default_anchor_positions, cluster_threshold = 7) -> clustered_default_anchor_positions
```
...
We can evem remove some of the poorly clustered groups to reduce the noise further.

```{r fig.height=7, fig.width=8}
plot_integration_anchors(
  split_objects,
  clustered_default_anchor_positions %>% filter(anchor_weight>400),
  reduction="umap.unintegrated",
) + theme_bw() 
```
