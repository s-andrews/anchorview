...
This is good but is overcrowded.  We can improve it by increasing the minimum score for the anchors to show
...
```{r}
cluster_anchor_positions(default_anchor_positions, min_score = 0, cluster_threshold = 7) -> clustered_default_anchor_positions
```
...
We can see that with these settings it's actually not doing a great job of matching up the correct cells.
